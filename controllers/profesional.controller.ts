...
export const obtenerProfesionalesPorId = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const profesional = await Profesional.findById(id);
    if (!profesional) return res.status(400).json({ error: "Plan no encontrado" });
    res.json(profesional);//✅Devuelve los profesionales
  } catch (error) {
    res.status(500).json({ error: "Error al obtener los profesionales por Id" })
  }
};
...
export const crearProfesionales = async (req: Request, res: Response) => {
  try {
    ...
    if (Array.isArray(req.body)) {
      const nuevaProfesion = await Profesional.insertMany(req.body, { ordered: true });
...
export const actualizarProfesionales = async (req: Request, res: Response) => {
  try {
    ...
    const actualizarProfesion = await Profesional.findByIdAndUpdate(id, updates, {
...
export const eliminarProfesionales = async (req: Request, res: Response) => {
  try {
    ...
    if (!profesional) return res.status(400).json({ error: "Profesional no encontrado" });
