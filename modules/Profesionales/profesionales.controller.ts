...
import * as service from "./profesionales.service";
...
export const getAll = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: "Error al obtener al profesional" });
  }
};
...
export const update = async (req: Request, res: Response) => {
  try {
    ...
    if (!update) {
      return res
        .status(404)
        .json({ error: "Profesional no encontrado para actualizar" });
    }
    ...
    res.status(500).json({ error: "Error al actualizar al profesional" });
  }
};
